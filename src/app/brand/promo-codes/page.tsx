...
export default function BrandPromoCodesPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const res = await api.get<PromoCode[]>("/promo-management/codes?source=brand");
      setCodes(res || []);
      setError(null);
    } catch (err: any) {
      setError(err.message || "Failed to load brand promo codes");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Promo codes linked directly to merchant and partner brand acquisitions."
        action={
      ...
      <Card className="overflow-hidden">
        {loading && codes.length === 0 ? (
          ...
          <div className="py-12 text-center text-sm text-[var(--muted)]">No brand promo codes generated yet.</div>
        ) : (
          <Table>
            <thead>
              <tr>
            ...
            <tbody>
              {codes.map((c) => (
                <tr key={c._id} className="hover:bg-black/5 transition-colors">
                  ...
                  <Td className="font-semibold text-[var(--ink)]">{c.brandName || "Brand Partner"}</Td>
