...
interface RevenueData {
  ...
  trends: {
    ...
    byTier: { tier: string; revenue: number }[];
    byCampaign: { campaign: string; revenue: number }[];
  };
}
...
export default function RevenuePage() {
  ...
  useEffect(() => {
    api.get<RevenueData>("/analytics/revenue").then(setData).catch((e) => setError(e.message));
  }, []);
  ...
  return (
    <div>
      <PageHeader title="Affiliate Revenue" subtitle="Revenue and ROI reporting for the entire affiliate channel — is it profitable?" />
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <KpiCard label="Avg Revenue / Affiliate" value={inr(data.totals.avgRevenuePerAffiliate)} accent="violet" />
      ...
      <div className="grid grid-cols-2 gap-5 mb-5">
        <Card className="p-5">
          <h2 className="font-display font-semibold text-[15px] mb-3">Revenue by Affiliate Tier</h2>
          <BarChart data={trends.byTier.map((t) => ({ label: t.tier, value: t.revenue }))} formatValue={inr} color="var(--amber)" />
        </Card>
        <Card className="p-5">
          <h2 className="font-display font-semibold text-[15px] mb-3">Revenue by Campaign</h2>
          {trends.byCampaign.length === 0 ? (
            <p className="text-sm text-[var(--muted)] py-6 text-center">No campaigns yet.</p>
          ) : (
            <BarChart data={trends.byCampaign.map((c) => ({ label: c.campaign, value: c.revenue }))} formatValue={inr} />
          )}
        </Card>
      </div>
