...
export default function CreatorPromoCodesPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const res = await api.get<PromoCode[]>("/promo-management/codes?source=creator");
      setCodes(res || []);
      setError(null);
    } catch (err: any) {
      setError(err.message || "Failed to load creator promo codes");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Creator & Influencer Promo Codes"
        subtitle="Exclusive discount coupons allocated to creators for social audience conversions."
        action={
      ...
      <Card className="overflow-hidden">
        {loading && codes.length === 0 ? (
          ...
          <div className="py-12 text-center text-sm text-[var(--muted)]">No creator promo codes found.</div>
        ) : (
          <Table>
            <thead>
              <tr>
