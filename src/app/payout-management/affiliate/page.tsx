...
export default function AffiliatePayoutsPage() {
  ...
  useEffect(() => {
    api.get<PayoutManagement[]>("/payout-management?partnerType=Affiliate").then(setRows).catch((e) => setError(e.message));
  }, []);
  ...
  return (
    <div>
      <PageHeader title="Affiliate Payouts" subtitle="The payout ledger filtered to affiliate partners." />
      <Card>
        {!rows ? (
          <Loading />
        ) : rows.length === 0 ? (
          <EmptyState title="No affiliate payouts yet" />
        ) : (
          <Table>
            <thead>
              <tr>
